...
async function main() {
    ...
    await initRenderer();
    const initialContent = loadFromHash() ??
...
async function updatePreview(ascii) {
    ...
    if (svgEl) {
        ...
        if (timeEl)
            timeEl.textContent = `${(t1 - t0).toFixed(1)}ms`;
        if (dimsEl && w && h)
...
function updateEditorStatus(content) {
    ...
    if (charCount)
        charCount.textContent = `${lines} lines · ${content.length} chars`;
}
function bindActions(editorView) {
