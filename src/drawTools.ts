...
export type DrawTool = 'select' | 'text' | 'box' | 'hline' | 'vline' | 'eraser';
...
interface BlockSelection extends Rect {
  lines: string[];
}

type SelectPhase =
  | { kind: 'idle' }
  | { kind: 'marquee'; anchor: Cell }
  | { kind: 'moving'; grab: Cell; snapshot: BlockSelection };

let activeTool: DrawTool = 'select';
let blockSel: BlockSelection | null = null;
let selectPhase: SelectPhase = { kind: 'idle' };
let overlayEl: HTMLElement | null = null;
let getView: (() => EditorView) | null = null;
let onChange: (() => void) | null = null;
let clipboard: string[] | null = null;
let pasteAnchor: Cell = { row: 0, col: 0 };
...
export function initDrawTools(
  ...
  for (const tool of Object.keys(TOOL_LABELS) as DrawTool[]) {
...
export function getActiveTool(): DrawTool {
  return activeTool;
}

export function focusTextTool(): void {
  setTool('text');
}
...
function cellAt(view: EditorView, x: number, y: number): Cell | null {
...
function rectFromBlock(b: BlockSelection): Rect {
  return { row0: b.row0, col0: b.col0, row1: b.row1, col1: b.col1 };
}

function isInk(ch: string | undefined): boolean {
  return ch !== undefined && ch !== ' ' && ch !== '\t';
}

/** Read marquee region without padding past end-of-line. */
function readRectLines(view: EditorView, r: Rect): string[] {
  const lines: string[] = [];
...
function tightenToInk(view: EditorView, r: Rect): Rect {
...
function extractBlock(view: EditorView, r: Rect): string[] {
  return normalizeLines(readRectLines(view, r));
}

function buildBlock(view: EditorView, r: Rect): BlockSelection {
 ...
 */
function applyBlock(view: EditorView, r: Rect, lines: string[]): void {
...
function clearBlock(view: EditorView, r: Rect): void {
  const changes: { from: number; to: number; insert: string }[] = [];
...
function drawBox(view: EditorView, r: Rect): void {
...
function drawHLine(view: EditorView, r: Rect): void {
  applyBlock(view, { ...r, row1: r.row0 }, ['-'.repeat(r.col1 - r.col0 + 1)]);
}

function drawVLine(view: EditorView, r: Rect): void {
  const lines = Array(r.row1 - r.row0 + 1).fill('|');
  applyBlock(view, { ...r, col1: r.col0 }, lines);
}

function eraseRect(view: EditorView, r: Rect): void {
  clearBlock(view, r);
}

function setOverlayContent(lines: string[] | null): void {
  if (!overlayEl) return;
  let pre = overlayEl.querySelector('.overlay-content') as HTMLPreElement | null;
  if (!lines) {
    pre?.remove();
...
function coordsForCell(
  ...
  return new DOMRect(
    coords.left,
    coords.top,
    Math.max((next?.right ?? coords.right + 7.8) - coords.left, 1),
    coords.bottom - coords.top,
  );
}

function showBlockSelection(view: EditorView): void {
  if (!blockSel) {
    updateOverlay(view, null);
    return;
  }
...
function pointInBlock(c: Cell, b: Rect): boolean {
...
function pointInBlockHit(c: Cell, b: BlockSelection): boolean {
  if (!pointInBlock(c, b)) return false;
  const ri = c.row - b.row0;
  const ci = c.col - b.col0;
  const line = b.lines[ri] ?? '';
  if (ci >= line.length) return false;
  if (isInk(line[ci])) return true;
  const contentW = line.replace(/\s+$/, '').length;
  return ci < contentW;
}

function movingRect(grab: Cell, cursor: Cell, snapshot: BlockSelection): BlockSelection {
...
function commitMove(view: EditorView, target: BlockSelection): void {
  applyBlock(view, rectFromBlock(target), target.lines);
  blockSel = { ...target };
}

function startMoveDrag(view: EditorView, cell: Cell): void {
  if (!blockSel) return;
  const snapshot: BlockSelection = {
    row0: blockSel.row0,
    col0: blockSel.col0,
    row1: blockSel.row1,
    col1: blockSel.col1,
...
function onMouseDown(e: MouseEvent): void {
  ...
  const view = getView?.();
  if (!view || activeTool === 'text') return;
...
function onMouseMove(e: MouseEvent): void {
  ...
  if (selectPhase.kind !== 'marquee') return;
...
function cancelMoveDrag(): void {
  const view = getView?.();
  if (!view || selectPhase.kind !== 'moving') return;
...
function moveBlockByKeys(dr: number, dc: number): void {
  const view = getView?.();
  if (!view || !blockSel) return;
  ...
  if (nr0 < 0 || nc0 < 0) return;

  const source: BlockSelection = { ...blockSel, lines: [...blockSel.lines] };
  const target: BlockSelection = {
    row0: nr0,
    col0: nc0,
    row1: nr0 + h,
    col1: nc0 + w,
...
function onKeyDown(e: KeyboardEvent): void {
  ...
  if (activeTool === 'select' && blockSel && selectPhase.kind !== 'moving') {
    ...
    if ((e.ctrlKey || e.metaKey) && e.key === 'v' && clipboard) {
      e.preventDefault();
      if (blockSel) clearBlock(view, rectFromBlock(blockSel));
    ...
    if (!e.ctrlKey && !e.metaKey && !e.altKey) {
      if (e.key === 'ArrowUp') { e.preventDefault(); moveBlockByKeys(-1, 0); return; }
      if (e.key === 'ArrowDown') { e.preventDefault(); moveBlockByKeys(1, 0); return; }
      if (e.key === 'ArrowLeft') { e.preventDefault(); moveBlockByKeys(0, -1); return; }
      if (e.key === 'ArrowRight') { e.preventDefault(); moveBlockByKeys(0, 1); return; }
...
export function refreshOverlay(): void {
  const view = getView?.();
  if (!view || selectPhase.kind === 'moving') return;
  if (blockSel) showBlockSelection(view);
}
