...
export class Controller {
  ...
  public handleMove(position: Vector, e: EventWithModifierKeys) {
    ...
    if (store.placeBlockTool.isActive && this.mode !== Mode.DRAG) {
      store.placeBlockTool.previewAt(moveCell);
      if (!moveCell.equals(this.lastMoveCell)) {
        setCanvasCursor(
          store.placeBlockTool.overlaps ? "not-allowed" : "crosshair"
        );
      }
      this.lastMoveCell = moveCell;
      return;
...
export class InputController {
  ...
  handlePointerDown = (e: React.PointerEvent<any>) => {
    ...
    if (this.pointers.size === 2) {
      ...
      } else if (store.placeBlockTool.isActive && e.button === 0) {
        store.placeBlockTool.place(screenToCell(pos));
      } else {
